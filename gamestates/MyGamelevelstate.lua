...
function MyGameLevelState:keypressed(key, scancode)
    ...
    if action == "pickup" then
        local sense = owner:getComponent(prism.components.Senses)
        if senses then
            local query = senses:query(prism.components.Carryable)
    ...
    if action == "inventory" then
        ...
        if inventory then
            self.manager:push(InventoryState(decision, self.levle, inventory))
        end
    end
end
